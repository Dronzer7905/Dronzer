...
logger = structlog.get_logger("dronzer.providers.mistral")


class MistralProvider(IProvider):
    """
    Mistral AI API integration (OpenAI compatible).
    """

    def __init__(self, default_base_url: str = "https://api.mistral.ai/v1"):
        self.default_base_url = default_base_url
        self._capabilities = ProviderCapabilities(
            chat=True,
            vision=False,  # Wait for Pixtral updates
            embeddings=True,
            images=False,
            streaming=True,
            json_mode=True,
            ...
            for item in data.get("data", []):
                models.append(
                    DiscoveredModel(
                        id=item["id"],
                        name=item["id"],
                        context_window=32768,  # Mistral default context length
