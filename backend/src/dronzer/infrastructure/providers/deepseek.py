...
logger = structlog.get_logger("dronzer.providers.deepseek")


class DeepSeekProvider(IProvider):
    """
    DeepSeek API integration (OpenAI compatible).
    """

    def __init__(self, default_base_url: str = "https://api.deepseek.com/v1"):
            ...
            for item in data.get("data", []):
                models.append(
                    DiscoveredModel(
                        id=item["id"],
                        name=item["id"],
                        context_window=65536,  # DeepSeek V2 context length
