...
logger = structlog.get_logger("dronzer.providers.groq")

class GroqProvider(IProvider):
    """
    Groq API integration (OpenAI compatible).
    Fast inference LPU platform.
    """

    def __init__(self, default_base_url: str = "https://api.groq.com/openai/v1"):
        ...
        async with httpx.AsyncClient() as client:
            ...
            for item in data.get("data", []):
                models.append(DiscoveredModel(
                    id=item["id"],
                    name=item["id"],
                    context_window=8192, # Groq defaults
