...
class KnowledgeSpace(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    settings: Mapped[dict] = mapped_column(
        JSON, default=dict
    )  # e.g. default embedding model, access policies

    # Relationships
    collections: Mapped[list["KnowledgeCollection"]] = relationship(
        "KnowledgeCollection", back_populates="space", cascade="all, delete-orphan"
    )
...
class Document(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    hash_checksum: Mapped[str] = mapped_column(
