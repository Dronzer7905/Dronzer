...
class MemorySession(Base, UUIDMixin, TimestampMixin):
    ...
    summary: Mapped[str | None] = mapped_column(Text, nullable=True) # Periodically updated rolling summary

    # Relationships
    turns: Mapped[list["MemoryTurn"]] = relationship("MemoryTurn", back_populates="session", cascade="all, delete-orphan")
...
class MemoryTurn(Base, UUIDMixin, TimestampMixin):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("memory_sessions.id", ondelete="CASCADE"), index=True)
