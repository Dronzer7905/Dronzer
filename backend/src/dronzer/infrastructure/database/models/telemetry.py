...
class RequestLog(Base, UUIDMixin, TimestampMixin):
    ...
    model_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("models.id", ondelete="SET NULL"), nullable=True)
...
class ModelHealth(Base, UUIDMixin, TimestampMixin):
    ...
    model_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("models.id", ondelete="CASCADE"), unique=True)
...
class KeyHealth(Base, UUIDMixin, TimestampMixin):
    ...
    api_key_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("api_keys.id", ondelete="CASCADE"), unique=True)
