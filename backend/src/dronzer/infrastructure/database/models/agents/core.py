...
class AgentProfile(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    supervisor_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("agent_profiles.id", ondelete="SET NULL"), nullable=True)
...
class AgentState(Base, UUIDMixin, TimestampMixin):
    ...
    agent_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("agent_profiles.id", ondelete="CASCADE"), index=True)
