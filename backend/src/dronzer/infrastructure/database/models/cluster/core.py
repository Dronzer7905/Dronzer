...
class ClusterNode(Base, UUIDMixin, TimestampMixin):
    ...
    cluster_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("clusters.id", ondelete="CASCADE"), index=True)
    ...
    capabilities: Mapped[dict] = mapped_column(JSON, default=dict) # e.g. {"gpu": true, "memory_gb": 64}
    labels: Mapped[dict] = mapped_column(JSON, default=dict)
