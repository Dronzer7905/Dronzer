...
class Prompt(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    owner_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("users.id")) # Assuming users exist
    ...
    versions: Mapped[list["PromptVersion"]] = relationship("PromptVersion", back_populates="prompt", order_by="desc(PromptVersion.created_at)")
