...
@router.get("/metrics")
async def get_metrics(request: Request, session: AsyncSession = Depends(get_db_session)) -> dict[str, Any]:
    """
    ...
    while current_hour <= end_hour:
        timeseries.append({
            ...
            "requests": data_dict.get(current_hour, 0)
        })
...
async def get_diagnostics(request: Request, session: AsyncSession = Depends(get_db_session)) -> dict[str, Any]:
    """
    ...
    providers_count = await session.execute(select(func.count(Provider.id)).where(Provider.is_active == True))
