...
async def create_organization(org: OrganizationCreate, session: AsyncSession = Depends(get_db_session)):
    """
...
async def get_organization(org_id: str, session: AsyncSession = Depends(get_db_session)):
    ...
    """
    result = await session.execute(select(Organization).where(Organization.id == uuid.UUID(org_id), Organization.is_deleted == False))
