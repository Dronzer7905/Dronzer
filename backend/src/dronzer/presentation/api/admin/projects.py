...
async def create_project(project: ProjectCreate, session: AsyncSession = Depends(get_db_session)):
    ...
    logger.info("Project created", project_id=str(db_project.id), org_id=str(db_project.organization_id))
...
async def get_project(project_id: str, session: AsyncSession = Depends(get_db_session)):
    ...
    """
    result = await session.execute(select(Project).where(Project.id == uuid.UUID(project_id), Project.is_deleted == False))
