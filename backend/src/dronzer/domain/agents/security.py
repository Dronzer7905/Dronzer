...
class AgentSecurityEnforcer:
    ...
    def validate_tool_execution(
        self, agent: AgentProfile, tool_name: str, parameters: dict[str, Any]
    ) -> bool:
        """
        ...
        if "*" not in agent.allowed_tools and tool_name not in agent.allowed_tools:
            ...
            raise PermissionError(
                f"Agent {agent.name} is not authorized to use the tool: {tool_name}"
            )
        ...
        for k, v in parameters.items():
            if isinstance(v, str):
                ...
                if any(bad_char in v for bad_char in [";", "&&", "|", "`", "$("]):
                    logger.warning(
                        "Potential command injection detected in tool parameters", param_key=k
                    )
