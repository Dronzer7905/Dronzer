        ...
        for policy in tenant_policies:
            ...
            if policy_type == "block_model":
                ...
                if request.model in blocked_models:
                    logger.warning(
                        "Policy Block: Model restricted",
                        model=request.model,
                        project=request.project_id,
                    )
                    raise PolicyViolationException(
                        f"The model '{request.model}' is blocked by Organization policy."
                    )
                ...
                if request.max_tokens > cap:
                    logger.warning(
                        "Policy Block: Max tokens exceeded cap",
                        requested=request.max_tokens,
                        cap=cap,
                    )
                    raise PolicyViolationException(
                        f"Requested max_tokens ({request.max_tokens}) exceeds the policy cap of {cap}."
                    )
                ...
                if request.provider not in allowed:
                    ...
                    raise PolicyViolationException(
                        f"Provider '{request.provider}' is not approved for this Organization."
                    )
