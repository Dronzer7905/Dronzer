...
class QuotaEngine:
    ...
    async def check_and_consume_rpm(self, tenant_id: str, limit: int) -> bool:
        ...
        if current and int(current) >= limit:
            ...
            raise QuotaExceededException(
                f"Rate limit of {limit} RPM exceeded for tenant {tenant_id}"
            )
    ...
    async def check_token_budget(
        self, project_id: str, max_tokens: int, estimated_tokens: int
    ) -> bool:
        """
        ...
        if current:
            ...
            if projected > max_tokens:
                logger.warning(
                    "Token Budget Exceeded", project=project_id, max=max_tokens, projected=projected
                )
                raise QuotaExceededException(
                    f"Token budget of {max_tokens} exceeded. Projected: {projected}"
                )
