...
class PrometheusMetrics:
    ...
    def __init__(self):
        ...
        self.active_connections = Gauge(
            "dronzer_active_connections",
            "Number of currently active SSE/WebSocket connections"
        )

        self.gateway_info = Info(
            "dronzer_gateway",
            "Dronzer AI Gateway Information"
        )
    ...
    def record_provider_call(self, provider: str, model: str, status: int, duration: float, prompt_tokens: int, completion_tokens: int, cost: float):
        ...
        if prompt_tokens > 0:
            self.tokens_consumed_total.labels(provider=provider, model=model, token_type="prompt").inc(prompt_tokens)
        if completion_tokens > 0:
            self.tokens_consumed_total.labels(provider=provider, model=model, token_type="completion").inc(completion_tokens)
