...
class FailoverEngine:
    ...
    async def execute_with_failover(
        ...
        try:
            ...
            return await execute_func(plan.primary_provider_id, plan.primary_model_id, plan.primary_key_id)
        ...
        for index, fallback in enumerate(plan.fallback_chain):
            ...
            if not provider_id or not model_id or not key_id:
                logger.error("Invalid fallback configuration missing required UUIDs", fallback=fallback)
                continue

            # Check if this fallback provider is even healthy before trying
            if not self.health_engine.is_provider_healthy(provider_id):
                logger.warning("Skipping fallback due to unhealthy provider circuit", provider=str(provider_id))
                continue
