        ...
        while True:
            try:
                ...
                if attempt >= max_attempts:
                    logger.error("Max retry attempts exhausted", attempt=attempt, max=max_attempts, exc_info=e)
                    raise e

                # Exponential backoff: 1s, 2s, 4s, etc.
                delay = (base_delay_ms * (2 ** (attempt - 1))) / 1000.0

                logger.warning("Upstream call failed, retrying", attempt=attempt, delay_sec=delay, error=str(e))
