...
async def seed():
    async for session in get_db_session():
        ...
        groq_provider = Provider(name="groq", base_url="https://api.groq.com/openai/v1", is_active=True)
        session.add(groq_provider)
        
        # Create Gemini Provider
        gemini_provider = Provider(name="gemini", base_url="https://generativelanguage.googleapis.com/v1beta/openai", is_active=True)
